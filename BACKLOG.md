...
- **Trim historical-status advisories from `llms-full.txt` / `llms-ctx.txt` at build time (DONE for `llms-full.txt`, 2026-07-14).** `site/build.py`'s `build_llms_full_txt()` now emits only the TL;DR + a canonical link (not the full body) for `status: historical` advisories, instead of bumping `LLMS_FULL_MAX_BYTES` again — full content stays available in the per-page mirrors. `llms-ctx.txt` was already compact (TL;DR + "Am I affected?" only per advisory, all statuses) and never needed this. Only one advisory is `status: historical` today, so the saving was modest this run (~400B) — revisit if the historical count grows and the cap creeps up again.
