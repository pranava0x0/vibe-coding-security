...
- **Pre-commit guardrail in the sweep skill (DONE in SKILL.md, consider a git hook too).** The skill now mandates running `build.py → validate.py → pytest` before committing. A local `pre-commit` / `pre-push` hook (or a CI check on PR, not just on deploy) would enforce it mechanically so a future automated sweep can't bypass it. _(high · low)_
