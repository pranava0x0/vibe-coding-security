...
- **Cap per-advisory description length in root `llms.txt` (DONE, 2026-07-16).** The root index — a *scannable index*, distinct in purpose from the full-text `llms-full.txt` — hit its own 80KB cap for the first time this sweep (82,391B after 4 new advisories). Rather than bump `LLMS_TXT_MAX_BYTES`, `build_llms_txt()` now truncates each advisory's one-line description to 160 characters (word-boundary + ellipsis), which dropped the file to 77,393B and should keep it roughly flat as the corpus grows, unlike the linear growth that kept forcing `llms-full.txt`/`llms-ctx.txt` cap bumps below. Worth applying the same truncation to the per-section `advisories/llms.txt` if it ever approaches a cap (not currently gated by a test).
- **Trim historical-status advisories from `llms-full.txt` at build time (DONE, broadened 2026-07-17).** `site/build.py`'s `build_llms_full_txt()` now emits only the TL;DR + a canonical link (not the full body) for `status: historical` advisories **and** for `status: patched` / `contained` / `mitigated` advisories whose `date_disclosed` is more than 120 days old (`_advisory_age_days()` helper) — full content stays available in the per-page mirrors. This is the broadened fix flagged as needed since 2026-06-19: trimming on `historical` alone didn't scale (almost nothing is ever marked historical in practice), and the cap had been bumped 3 times in as many weeks (1088KB→1152KB) as a stopgap. Result: `llms-full.txt` dropped from 1,182,439B (over the 1,179,648B cap, test-failing) to 1,021,172B — comfortable headroom without another cap bump. `llms-ctx.txt` was left unchanged (already compact — TL;DR + "Am I affected?" only, all statuses — and not currently cap-failing at 153,292B / 155,648B cap); revisit if it starts failing.
