...
- **Codex review (PR #85) caught the first draft overstating the risk in three places** — RLS-enabled-with-zero-policies is default-deny for ordinary roles, not a leak (the real risk is fragility + bypass-role credentials); anon/publishable keys are meant to be public and shouldn't be blanket-"rotated" (pausing/deleting the project is what actually matters; rotation is for secret/service-role keys); and wildcard CORS only "travels" to a new route if the same header rule's path matcher covers it, not automatically by shared origin. Corrected in all four touched files before merge. Root cause: content bug — plausible-sounding technical claims that didn't hold up to a security-focused review.
