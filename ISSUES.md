...
- **Codex review (PR #85) caught the first draft overstating the risk in three places, then a second pass after those fixes caught three more.** Round 1: RLS-enabled-with-zero-policies is default-deny for ordinary roles, not a leak (the real risk is fragility + bypass-role credentials); anon/publishable keys are meant to be public and shouldn't be blanket-"rotated" (pausing/deleting the project is what actually matters; rotation is for secret/service-role keys); wildcard CORS only "travels" to a new route if the same header rule's path matcher covers it, not automatically by shared origin. Round 2 (only visible once round 1's fixes had landed): the "default-deny, not an error" claim was read-only — a write against a policy-less RLS table fails loudly with a real RLS-violation error, and that's the more likely trigger for someone disabling RLS as a "fix"; a catch-all CORS matcher covering a future endpoint isn't automatically exploitable — depends on credential mode (cookie vs. bearer-token) and CORS was never a CSRF substitute anyway; and `ALERTS.md`'s exposure-pattern summary still grouped RLS-enabled-no-policy under a "data exposure" heading despite the corrected framing, which could make a default-deny (safe) table read as exposed. All six corrected in the touched files before merge. Root cause: content bug — plausible-sounding technical claims that didn't hold up to a security-focused review, including on the corrections themselves.
