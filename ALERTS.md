...
>
> **Last refreshed:** 2026-06-10. If this date is more than 7 days old, treat the repo as stale — check [sources/](sources/) directly.

---

## 🔴 ACTIVE — react now

### 2026-06-08 — Hades — 19 bioinformatics + ~23 MCP developer PyPI packages steal Anthropic/cloud/GitHub creds via .pth + Bun
Socket's Threat Research Team flagged **Hades**, the newest Shai-Hulud worm family wave (June 8–9, 2026): **19 bioinformatics PyPI packages** (Dynamo, Spateo, CoolBox, U-FISH, Napari-UFISH, embiggen, ensmallen, gpsea, phenopacket-store-toolkit, pyphetools, ppkt2synergy) and **~23 MCP developer packages** embed a **`*-setup.pth`** startup hook that fires at every Python interpreter startup via the `site` module — no `import`, no explicit call. The hook silently downloads the **Bun JavaScript runtime from GitHub** (bypasses static analysis of the wheel itself) and runs a credential-stealing `_index.js` payload harvesting **Anthropic API keys, `~/.claude/` MCP configs, GitHub/npm/PyPI tokens, AWS/GCP/Azure credentials, SSH keys, and `.env` files**, exfiltrated to attacker-controlled GitHub repositories. The bioinformatics sub-cluster adds trojanized native **`.abi3.so` CPython extensions** as a fallback delivery path that fires at `import` time. Typosquat packages (`rsquests`, `tlask`, `rlask`) are also included. **Total campaign so far: 471 malicious artifacts (411 npm, 60 PyPI) across 37 PyPI packages** in the Shai-Hulud family umbrella. If you installed any named package after June 8: uninstall it, delete `*-setup.pth` and `_index.js` from site-packages, and rotate all listed credentials.
→ [advisories/2026-06-hades-pypi-worm.md](advisories/2026-06-hades-pypi-worm.md)
...
### 2026-05 — Claude Code MCP OAuth token interception via malicious npm postinstall hook (Mitiga Labs; Anthropic: "out of scope")
Mitiga Labs found that a malicious npm **`postinstall`** hook can rewrite **MCP server URLs in `~/.claude.json`**, routing Claude Code's OAuth authorization flows through an attacker-controlled proxy. OAuth tokens for connected services (Jira, Confluence, GitHub, Slack, etc.) are stored **in plaintext** in `~/.claude.json` and can be harvested directly or intercepted mid-flow. A **persistence mechanism** restores the malicious URL if the user edits it or rotates the token — the config silently reverts on next load. Reported to Anthropic April 10, 2026; Anthropic responded April 12: **"out of scope"** (the user consented to install the malicious package). **No patch, no CVE.** The attack surface grows as more MCP servers adopt OAuth authorization flows. Any machine where Claude Code has MCP OAuth configured and where any npm package has been installed from an unverified source is at risk. Audit `~/.claude.json` for unexpected MCP server URLs; revoke all OAuth tokens for connected services; rotate credentials.
→ [advisories/2026-05-claude-code-mcp-traffic-hijacking.md](advisories/2026-05-claude-code-mcp-traffic-hijacking.md)
...
### 2025-10-17 — GlassWorm — self-propagating VS Code / Open VSX worm (CrowdStrike+Google+Shadowserver takedown 2026-05-26)
First self-propagating worm in VS Code/Open VSX extensions. Hides payload in **invisible Unicode** (literally unreadable in an editor); C2 was **quad-redundant** — Solana blockchain dead-drop + BitTorrent DHT + Google Calendar dead-drops + direct VPS IPs. Stole npm/GitHub/Git creds (poisoning **300+ GitHub repos** alone), drained 49 crypto wallets, dropped SOCKS proxies + hidden VNC, re-seeded itself. Multiple 2026 waves (Dec 2025; 72+ Open VSX extensions since Jan 31; v2 Mar–Apr hitting 150+ GitHub repos; 73 "sleeper" extensions in late April). On **2026-05-26 14:00 UTC**, **CrowdStrike Counter Adversary Operations + Google + Shadowserver Foundation** disabled **all four C2 channels simultaneously** — likely Russia-based operator (CIS-locale exit). Status: **mitigated, not eradicated** — poisoned extensions/packages still on disk; rotate credentials if you ever installed a flagged extension. Almost certainly fed the [Megalodon](advisories/2026-05-megalodon-github-actions-mass-campaign.md) credential pool.
