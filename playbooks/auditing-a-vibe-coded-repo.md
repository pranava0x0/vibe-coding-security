...
Also check hosting/framework **header rules**, not just app code — `vercel.json`, `netlify.toml`, a framework's static-export config. CORS headers attach per matched route, they don't spread across an origin on their own — check the rule's **path matcher**: a wildcard scoped to genuinely static paths is harmless, while a global/catch-all matcher (e.g. `"source": "/(.*)"`) means any endpoint added later under that host is covered by the same rule the moment its path matches. Whether that's actually exploitable depends on how the endpoint authenticates, not just on the wildcard existing: browsers already refuse to honor a wildcard `Access-Control-Allow-Origin` on a **cookie-based, credentialed** request, so a catch-all wildcard mostly fails closed there. It's a real problem for **bearer-token** endpoints, where a wildcard origin combined with a token any page's JS can read lets an attacker's page make the request and read the response. Either way, don't treat CORS as a CSRF control: a state-changing endpoint needs its own anti-CSRF check regardless of the CORS config, since a simple (non-preflighted) cross-origin request can still be sent whether or not the response can be read back.
