...
- **AI-agent config files (`.cursorrules` / `CLAUDE.md` / `AGENTS.md`) are a write-target, not just a read-target.** Earlier incidents *read* AI-tool config to steal creds (Bitwarden CLI AI-tool collector; Nx Console `~/.claude/settings.json` theft). TrapDoor (May 2026) inverts it: a malicious package **writes a hidden prompt-injection instruction into the repo's `.cursorrules` / `CLAUDE.md`**, hidden with **zero-width Unicode**, so the developer's *own* agent re-runs the attack on its next invocation (exfiltration disguised as an "automated security scan"). This fuses the GlassWorm steganographic-Unicode technique with AI-config-as-persistence/re-infection. Triage cues: on any supply-chain or repo-compromise finding, ask **"did it touch an agent-instruction file?"**; offer the grep-for-invisible-Unicode heuristic over `.cursorrules`/`CLAUDE.md`/`AGENTS.md`/`.github/copilot-instructions.md`/`.windsurfrules`; and advise readers to keep those files in version control and diff them on every dependency change. This is a vibe-coding-specific surface — write such incidents up even when the headline is "another crypto stealer," because the AI-config persistence is the part our audience won't see coming.
