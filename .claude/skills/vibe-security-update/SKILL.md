...
- **Poisoned IDE/editor extension as a platform-breach vector.** A malicious version of an editor extension (VS Code / Cursor / Windsurf / Open VSX) on a single developer's machine is a foothold into every repo and credential that editor touches. TeamPCP used exactly this to exfiltrate ~3,800 of GitHub's *own* internal repos in May 2026. Same trust-transfer shape as the Amazon Q wiper and postmark-mcp (build trust over clean versions, then push one poisoned release) and a sibling of the AI-tool OAuth-pivot class. When triaging an "AI/dev tool was compromised" story, check whether the delivery vector was an extension marketplace, and whether silent auto-update carried the payload.
