...
**Update 2026-09-09 — two authorization-bypass CVEs, both medium severity: the same "which permission gate did I check" mistake in two different execution paths.**

- **CVE-2026-86996 / GHSA-7hgx-277f-7vmg** (CVSS 3.1 6.4, Medium, CWE-862 Missing Authorization). The **"This workflow can be called by"** setting that restricts who may invoke a workflow was enforced by the Execute Workflow node — but not when the same workflow was attached to an **Agent as a tool** (`packages/cli/src/modules/agents/tools/workflow-tool-factory.ts` omitted the `SubworkflowPolicyChecker.checkForProject` call that the Execute Workflow path makes). A user able to build an Agent could therefore call — and read back the output of — a workflow its owner had explicitly restricted. Affects **< 2.37.7** and **2.38.0–2.38.1**; fixed in **2.37.7 / 2.38.2**. Directly relevant to this repo's audience: it's an AI-agent-tool code path, not the conventional workflow-execution path, that missed the check.
- **GHSA-h3jj-5f3v-3685** (CVSS 6.4, Medium, CWE-863 Incorrect Authorization, no CVE assigned). The n8n **Public API**'s execution-retry endpoint authorized on `workflow:read` instead of `workflow:execute`, letting a user with only read access to a shared workflow trigger execution retries through the API — bypassing the read/execute permission separation. Affects **< 2.25.7** and **2.26.0–2.26.1**; fixed in **2.25.7 / 2.26.2**.

Both were confirmed directly against GitLab's Advisory Database mirror of the underlying GHSA pages rather than an aggregator summary. Neither is part of the same disclosure batch as the 2026-08-19 nine-advisory set above — they entered the advisory database independently on 2026-09-02 (GHSA-h3jj-5f3v-3685) and 2026-09-08 (CVE-2026-86996/GHSA-7hgx-277f-7vmg). *Correction, 2026-09-13:* GHSA-7hgx-277f-7vmg was in fact one of the eighteen advisories in n8n's **2026-09-02** batch (below); the 09-08 date is when its CVE reached the advisory database.

**Update 2026-09-13 — the 2026-09-02 eighteen-advisory batch: two more expression-sandbox escapes to code execution in the n8n process, an unauthenticated storage-exhaustion DoS, and an AI-node domain-restriction bypass.** n8n's community-forum security post of 2026-09-02 (Matthew von Zweigbergk) enumerates **eighteen** advisories — **five High, thirteen Medium** — all fixed in **1.123.76 / 2.37.7 / 2.38.2**. The five High, each verified on its own GHSA page:

- **CVE-2026-86076 / GHSA-hw8v-xxg5-vvvx** (CVSS 8.7) — **Expression sandbox escape via class-field sanitizer rebinding.** The expression compiler's sanitizer was resolved through a dynamically-scoped `this` and did not reject reserved class-member names, so a class field named `__sanitize` rebinds the sanitizer and reaches the `Function` constructor. On the backend, **any expression author runs code in the n8n process**; in the editor preview, a member's expression runs as JavaScript in the browser session of whoever opens the workflow. Reporter Masofgon. Third sandbox-escape-to-RCE in the expression engine this file tracks (after CVE-2026-27577 and GHSA-fg85-4wv2-p98j); the fix rejects reserved class-member names.
- **CVE-2026-86083 / GHSA-6xcw-7xm6-48c6** (CVSS 7.7) — **Expression sandbox escape via shared-builtin tampering and code-printer injection.** The *legacy* expression engine called the mutable global `JSON.stringify` while printing string literals and interpolating timezone data into generated source; replacing that global from an expression turns data literals into executable code. Arbitrary code execution in the n8n process. **Legacy engine only** — instances on the default `vm` engine are not affected, and `N8N_EXPRESSION_ENGINE=vm` is the vendor's listed workaround. Reporter hiddingtrojans.
- **CVE-2026-86075 / GHSA-hh89-3r9w-qj3j** (CVSS 8.7) — **Unauthenticated persistent storage exhaustion via the OAuth Dynamic Client Registration endpoint.** `client_name` and `grant_types` were persisted with no size validation, so an attacker "without any account on the instance" registers clients in a loop until the database fills. **Affects 2.x only** (`< 2.37.7`, `< 2.38.2`). Reporter Masofgon. Relevant to this repo because the DCR endpoint exists to let MCP clients self-register — it is the agent-onboarding surface, and it was reachable pre-auth.
- **CVE-2026-86081 / GHSA-j535-v25q-vx3q** (CVSS 7.1) — ReDoS in the Git node's default blocked-file-pattern match via a crafted clone destination path; the match runs synchronously in the main process, so "an authenticated user could freeze the instance for everyone with a single workflow execution." Fourth distinct Git-node finding in this file. Reporter Masofgon.
- **CVE-2026-86082 / GHSA-34ff-336r-5q23** (CVSS 7.1) — **Domain-restriction bypass in the OpenAI Chat Model node**: the custom-base-URL allowlist that administrators configure was enforced on completions but not on the model-search dropdown, so requests — with the credential attached — reach arbitrary hosts. Reporter yadhukrishnam. The fix centralizes every OpenAI call behind one validation layer.

The thirteen Medium, by title from the forum post (GHSA ids there; not individually fetched): disabled OIDC SSO endpoints still issuing sessions (GHSA-pf83-w3f9-8m37); anonymous approval-gate bypass via `resumeToken` reuse over the chat WebSocket (GHSA-35jj-42hp-8gmq); log-streaming destination credential decryption without ownership checks (GHSA-pq6c-vh67-xpm3); cross-tenant project-member PII disclosure (GHSA-cqr2-h44g-v75v); cross-user active-workflow-ID disclosure (GHSA-65xw-2v52-jhxc); prototype pollution via workflow structure summary (GHSA-679f-58pq-4v2c); Git node `branch..remote` config sandbox bypass (GHSA-qgpw-8g46-w95v); path injection in the Elasticsearch/ElasticSecurity nodes (GHSA-f2cp-m7mv-8jpv); instance AI-credential setup URL validation failure (GHSA-q5wm-mgqx-fv2f); GitHub Trigger webhook-secret bypass on 422 reuse (GHSA-5m98-cgcr-xx3q); the Agent-tool sub-workflow caller-policy bypass already tracked above (GHSA-7hgx-277f-7vmg); cross-project workflow/credential deletion via source-control push (GHSA-hvrx-jc5j-pg3w); and a Git node file-sandbox escape via relative URLs (GHSA-fm93-2x43-6676).
...
**2026-09-13 update sources** — the batch index plus the five High advisories, each fetched directly from n8n's GHSA index:
- [n8n Community — Security update — 02 September 2026](https://community.n8n.io/t/security-update-02-september-2026/310852) — fetched 2026-09-13; the vendor's own batch post listing all eighteen advisories with severities and the 1.123.76 / 2.38.2 / 2.37.7 fixed versions.
- [GHSA-hw8v-xxg5-vvvx (CVE-2026-86076)](https://github.com/n8n-io/n8n/security/advisories/GHSA-hw8v-xxg5-vvvx) — class-field sanitizer rebinding, CVSS 8.7, backend RCE and editor-preview JS execution.
- [GHSA-6xcw-7xm6-48c6 (CVE-2026-86083)](https://github.com/n8n-io/n8n/security/advisories/GHSA-6xcw-7xm6-48c6) — `JSON.stringify` tampering in the legacy engine, CVSS 7.7, `N8N_EXPRESSION_ENGINE=vm` workaround.
