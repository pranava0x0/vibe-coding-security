...
3. **Update nginx-ui to ≥ 2.3.4** (CVE-2026-33032 / MCPwn), or add `middleware.AuthRequired()` to `/mcp_message` and set the IP allowlist to deny-all.
4. **For Apache Doris MCP:** apply the patch + the CVE tracker recommendations.
5. **For Alibaba RDS MCP:** since Alibaba declined to patch, do not deploy without an isolating proxy that filters MCP messages.
6. **Do not expose stdio MCPs to network sockets.** If you've wrapped one with a proxy / HTTP gateway, audit the proxy's input validation.
7. **Run MCP servers as their own unprivileged user**, in a container with read-only credentials, on a host with strict egress allowlists.
