...
tools_affected: [openclaw, "@openclaw/whatsapp", "@openclaw/matrix", "@openclaw/slack / discord / feishu / msteams / googlechat / voice-call", "OpenClaw for iOS", "ClawHub (openclaw/clawhub backend)", NemoClaw, ClawHub skills]
...
## Update 2026-09-24 — the vendor names the source of the batch: a Trail of Bits audit under OpenAI's Patch the Planet (27 advisories, 2 High), all fixed in 2026.8.1 and the 2026.7.33 LTS; plus a low-severity canvas DoS still open in 2026.9.5

OpenClaw's blog post of **2026-09-21**, "OpenClaw Completes Security Audit Through OpenAI's Patch the Planet Initiative," explains where the 09-11 bulk publication came from: Trail of Bits, working with Codex-assisted workflows under OpenAI's open-source patching programme, "submitted 27 private repository advisories and 3 standalone hardening pull requests"; 24 of the 27 carried severity ratings — **0 Critical, 2 High, 16 Medium, 6 Low** — 23 were confirmed, one had been fixed before submission, and the other three were defense-in-depth findings with no rating "because they did not cross a documented trust boundary." The **two Highs are the two already at the top of this file**: exec approvals reusable in a working directory nobody reviewed (GHSA-3mq7-q27j-mq7q) and the WhatsApp login tool reachable from non-owner turns (GHSA-9m4p-cqp4-jppq). The vendor's own summary of the dominant bug class — "losing permissions between steps," where "a request might enter OpenClaw with limited access, then start another piece of work that no longer carried those limits" — is the authorization-inheritance failure this file's TL;DR describes from the outside. Fix status per the vendor: every issue "repaired on `main` and … shipped in the **2026.8.1** and **2026.7.33 LTS** stable releases." That is the first statement naming the LTS line as also fixed; the upgrade guidance above now reads "≥ 2026.8.1, or ≥ 2026.7.33 on the LTS train."

Separately, the advisory database's `openclaw` query shows **CVE-2026-94094** (GHSA-24pq-v99h-hj63, VulDB-sourced, published 2026-09-21, CVSS 4.0 2.1 Low): the canvas extension's host route (`createCanvasHostHandler` in `extensions/canvas/src/host/server.ts`) buffers whole files with `readFile()` — a remote, low-privilege denial of service — and the record states the suggested streaming/size-limit fix "was never shipped — latest 2026.9.5 still buffers the whole file." Low impact, noted here so the next sweep does not re-triage it.


## Update 2026-09-26 — VulnCheck assigns 77 CVEs to the batch overnight (CVE-2026-100525 … CVE-2026-100604), including a **9.0 for the iOS app** and five for the ClawHub registry backend; several fixes land *after* 2026.8.1, so the upgrade target is now **≥ 2026.9.3** (iOS ≥ 2026.8.11)

Fifteen days after the 75-advisory dump, the CVE wave arrived: an NVD query for `openclaw` published 2026-09-24 → 09-26 returns **77 records, all published 2026-09-26, all with CNA `disclosure@vulncheck.com`**, numbered contiguously **CVE-2026-100525 through CVE-2026-100604** (two numbers in the range are not OpenClaw's). Each carries a CVSS 4.0 score VulnCheck assigned; the vendor advisories carried only a severity word plus, in some cases, a CVSS 3.1 number, and the two disagree in the usual direction — the Claude-permission-prompt bug the 09-14 walk singled out ([GHSA-p5g8-m35v-7m82](https://github.com/openclaw/openclaw/security/advisories/GHSA-p5g8-m35v-7m82), vendor High / 8.0) is **CVE-2026-100585, VulnCheck 8.6** ("an authorized non-owner channel sender with channel command access can approve or deny a pending permission request intended for the owner"). The advisory database mirrors each CVE as a *second* entry beside the vendor advisory (e.g. [GHSA-87g7-76gr-73g8](https://github.com/advisories/GHSA-87g7-76gr-73g8) for the same bug), so a GHSA grep will find two ids per issue from now on — grep the CVE.

What the scores add, ranked by VulnCheck's number:

| CVE | CVSS 4.0 | Affected → fixed | What it is (from the NVD description) |
|---|---|---|---|
| CVE-2026-100551 | **9.0** | OpenClaw for iOS ≥ 2026.7.1, < **2026.8.11** | the Control UI did not enforce saved Gateway TLS pins — an on-path attacker can impersonate the gateway to the phone app |
| CVE-2026-100567 | 8.9 | ≥ 2026.4.5, < 2026.8.1 | gateway authorization gap (an agent-gateway request path that skipped the sender's policy) |
| CVE-2026-100597 | 8.8 | < 2026.7.1 | TOCTOU race in an approval path |
| CVE-2026-100544 | 8.7 | `@openclaw/voice-call` < 2026.8.1 | classic inbound voice calls launched the configured agent for any caller |
| CVE-2026-100552 | 8.7 | < 2026.8.1 | per-chat tool policies not enforced for Codex sessions |
| CVE-2026-100557 / -100558 / -100568 / -100575 / -100586 / -100587 / -100588 / -100589 / -100596 / -100599 | 8.7 each | mostly < 2026.7.1 or < 2026.8.1 | skill-tool dispatch authorization bypass; Gateway listener resource exhaustion; operator cron-job access; Slack multi-person DM allowlists; Codex native-conversation and computer-use owner checks; browser-tool admin scope and sandbox bypass; **non-owner MCP configuration changes** (stdio command → host code execution, the finding this file's TL;DR led with); Google Meet nodes skipping the exec-approval path |
| CVE-2026-100559 / -100561 / -100585 | 8.6 | < 2026.8.1 / ≥ 2026.3.22 < 2026.8.1 / < 2026.7.1 | escaped newlines confusing the exec command parser; an approval-bypass in the exec path; the Claude permission-prompt bug above |
| CVE-2026-100530 / -100570 | 8.5 | < 2026.8.1 | reusable exec approvals not bound to the working directory ("Exec approvals could outlive their reviewed working directory" on the vendor tab); an untrusted workspace `.env` file honoured by the runtime |
| CVE-2026-100603 | 8.7 | ClawHub backend (no version) | four distinct flaws in the **skill report moderation flow** — the registry's own abuse-handling path |
| CVE-2026-100600 … -100604 | 5.3–8.7 | ClawHub backend | anonymous API requests not bound to a trusted caller; SSRF in the publish path; a missing authorization check on changelog preview; the moderation flaw above; an incorrect-authorization bug |

**Fix versions past 2026.8.1 (the number this file has been recommending):** CVE-2026-100527 (browser-relay DoS) is fixed in **2026.8.2**; CVE-2026-100546 (a Discord race) affects ≥ 2026.7.2 and is fixed in **2026.9.2**; CVE-2026-100525 (`@openclaw/diagnostics-prometheus`) and CVE-2026-100526 (`@openclaw/discord`) are fixed in **2026.9.3**; the two iOS CVEs (100551, 100581 — cleartext gateway credentials in App Group UserDefaults) are fixed in **iOS 2026.8.11**. Registry check 2026-09-14 already showed 2026.9.4 as `latest`, so a routine upgrade covers all of them; a pinned 2026.8.1 does not. The ClawHub backend CVEs have no version field — they describe the hosted registry, which the project operates, so there is nothing for a user to upgrade but there is something to know: the skill-report/moderation path this corpus relies on when a malicious skill is reported ([ClawHavoc](2026-02-clawhavoc-clawhub-skills.md)) had authorization holes of its own.

Status stays `patched`; severity stays `high` (one 9.0 in a client app whose gateway-impersonation impact depends on the attacker being on-path, not the 9.x-unauthenticated-RCE shape this repo reserves `critical` for). The "am I affected" step is unchanged, with the target version raised: **`openclaw --version` ≥ 2026.9.3, iOS app ≥ 2026.8.11**, and audit non-owner channel activity for the window before your upgrade.
...
- **2026-09-24 update sources** — [OpenClaw — OpenClaw Completes Security Audit Through OpenAI's Patch the Planet Initiative](https://openclaw.ai/blog/openclaw-trail-of-bits-engagement-recap) (vendor, 2026-09-21: the 27/3 counts, the 0/2/16/6 severity split, the permission-loss finding, the 2026.8.1 and 2026.7.33 LTS fix statement). Fetched 2026-09-24. [GitHub Advisory Database — GHSA-24pq-v99h-hj63 (CVE-2026-94094)](https://github.com/advisories/GHSA-24pq-v99h-hj63) (fetched 2026-09-24: canvas host DoS, CVSS 2.1, "never shipped" note, affected through 2026.9.5).

**2026-09-26 update sources** — all fetched 2026-09-26:
- [NVD API — CVEs matching `openclaw`, published 2026-09-24 → 09-26](https://services.nvd.nist.gov/rest/json/cves/2.0?keywordSearch=openclaw&pubStartDate=2026-09-24T00:00:00.000&pubEndDate=2026-09-26T23:59:59.000&resultsPerPage=200) — 77 results, CNA `disclosure@vulncheck.com`, the CVSS 4.0 scores, affected/fixed ranges and descriptions tabulated above.
- [VulnCheck — OpenClaw before 2026.7.1 authentication bypass via MCP channel (CVE-2026-100585)](https://www.vulncheck.com/advisories/openclaw-before-2026.7.1-authentication-bypass-via-mcp-channel) — the CNA record: 8.6, CWE-862, fixed 2026.7.1, credit whyiug, reference to the vendor advisory.
- [openclaw/openclaw — GHSA-p5g8-m35v-7m82, "Claude permission replies could omit owner authorization"](https://github.com/openclaw/openclaw/security/advisories/GHSA-p5g8-m35v-7m82) — the vendor advisory for the same bug: High / CVSS 8.0, published 2026-09-11, "no known CVE" still shown on the page at fetch time.
- [GitHub Advisory Database — GHSA-87g7-76gr-73g8 (CVE-2026-100585)](https://github.com/advisories/GHSA-87g7-76gr-73g8) — the database's CVE-sourced mirror entry, published 2026-09-26, showing the two-ids-per-bug shape; the `mcp`, `claude` and `agent` recency listings each carried 20+ of the wave on 2026-09-26.
